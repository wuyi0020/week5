<template>
    <div class="card justify-content-between h-100 text-center border-dark p-0">
        ...
        <div class="card-body d-flex align-content-between flex-column">
            ...
            <img v-if="edittingList[item.id]" :src="editItem[item.id].imageUrl" class="mb3 flex-grow-1" alt="" />
            <img v-else :src="item.imageUrl" alt="" class="coin mb-3 flex-grow-1 card-img-top" />

            <div class="input-group align-self-end mb-3" v-if="edittingList[item.id]">
                <div class="input-group-prepend">
            ...
            <div class="input-group mb-3" v-if="edittingList[item.id]">
                <div class="input-group-prepend">
                    <span class="input-group-text" :id="`${item.id}_order`">排序</span>
                </div>
                <input type="number" class="form-control" :placeholder="item.order" aria-label="排序"
                    :aria-describedby="`${item.id}_order`" v-model.number="editItem[item.id].order" />
            </div>
        </div>
        <div class="card-footer">
            ...
            <div>
                <button v-if="!edittingList[item.id]" key="修改" class="btn btn-outline-success mx-1"
